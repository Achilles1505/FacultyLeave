	...
	<body>
		<?php
			...
			if(isset($_POST["apply"])){
				...
				if($status==0){
					...
					$sql = 'SELECT dept_id FROM faculty_details WHERE fid = '.$user.'';
					$result = mysqli_query($conn, $sql);
					$row = mysqli_fetch_assoc($result);
					$sql = 'SELECT dept_name FROM department WHERE dept_id = "'.$row["dept_id"].'"';
					$result = mysqli_query($conn, $sql);
					$row = mysqli_fetch_assoc($result);
					...
					$sql = 'INSERT INTO pending_tasks VALUES ("HOD '.$dept_name.'", '.$app_no.', NULL, FALSE)';
					if(mysqli_query($conn, $sql))
						$status=0;
					else
						$status=4;
					
					$sql = 'INSERT INTO work_flow VALUES ("start", '.$app_no.', "HOD '.$dept_name.'")';
					if(mysqli_query($conn, $sql)){
		...
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<?php
						$sql = 'SELECT firstname FROM faculty_details WHERE fid = '.$user.'';
						$result = mysqli_query($conn, $sql);
