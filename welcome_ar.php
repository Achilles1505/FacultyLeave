		...
		<?php
			...
			if(isset($_POST["apply"])){
				...
				if($status==0){
					...
					$sql = 'INSERT INTO pending_tasks VALUES ("Dean", '.$app_no.', NULL, FALSE)';
					if(mysqli_query($conn, $sql))
						$status=0;
					else
						$status=4;
    				
					$sql = 'INSERT INTO work_flow VALUES ("start", '.$app_no.', "Dean")';
					if(mysqli_query($conn, $sql))
							...
							<div class="tab-content">
									...
									<div class="container" style="width: 95%">
										<div class="panel-group" id="accordion">
											<?php
												...
												$sql = 'SELECT app_no FROM pending_tasks WHERE role = "Assistant Registrar" and complete = FALSE';
												$result = mysqli_query($conn, $sql);
												$i=0; $j=0;
												while($row = mysqli_fetch_assoc($result)){
													if(isset($_POST['forward'.$i.''])){
														...
														$sql = 'INSERT INTO pending_tasks VALUES ("Dean", '.$row["app_no"].', NULL, FALSE)';
														if(mysqli_query($conn, $sql))
															echo "Inserted in pending tasks";
														else
															echo "Could not insert into pending tasks";
														
														$sql = 'UPDATE work_flow SET role1 = "Assistant Registrar", role2 = "Dean" WHERE app_no = '.$row["app_no"].'';
														if(mysqli_query($conn, $sql))
													...
													?>
														...
														echo '</table>';
													?>
																		<form action = "welcome_ar.php" method = "post">
																		<?php
								...
								<?php
									if(isset($_POST["assign"])){
										...
										$sql = 'INSERT INTO roles VALUES ("Assistant Registrar", '.$_POST["new_role"].', "'.$_POST["start_role"].'", "'.$_POST["end_role"].'", '.$user.', '.$old_role["role"].')';
										if(mysqli_query($conn, $sql)){
											$sql = 'UPDATE faculty_details SET role = "Assistant Registrar" WHERE fid = '.$_POST["new_role"].'';
