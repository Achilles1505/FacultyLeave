						...
						<div class="tabbable-line">
							...
							<div class="tab-content">
								<div class="tab-pane active" id="tab_default_1">
									...
									<div class="container" style="width: 95%">
										<div class="panel-group" id="accordion">
											<?php
												$sql = 'SELECT app_no FROM pending_tasks WHERE role = "Dealing Clerk" and complete = FALSE';
												$result = mysqli_query($conn, $sql);
												$i=0; $j=0;
												while($row = mysqli_fetch_assoc($result)){
													...
													if(isset($_POST['forward'.$i.''])){
														...
														if(mysqli_query($conn, $sql)){
															...
															echo "Pending Tasks did not change.";
																...
																$sql = 'INSERT INTO pending_tasks VALUES ("Assistant Registrar", '.$row["app_no"].', NULL, FALSE)';
																if(mysqli_query($conn, $sql))
																	echo "Inserted in pending tasks";
																else
																	echo "Could not insert into pending tasks";
														
															$sql = 'UPDATE work_flow SET role1 = "Dealing Clerk", role2 = "Assistant Registrar" WHERE app_no = '.$row["app_no"].'';
															if(mysqli_query($conn, $sql))
													...
													?>
														...
														echo '</table>';
													?>
																		<form action = "welcome_dealingclerk.php" method = "post">
																		<?php
																			if(strcmp($role1["role1"], "Dean")==0 || strcmp($role1["role1"], "Director")==0)
																				echo '<button type = "submit" class = "button" name = "approve'.$i.'">Update Record</button>';
																			else
								...
								<div class="tab-pane" id="tab_default_4">
									...
									Filter using:
									<form action = "welcome_faculty.php" method = "post">
										<?php
										...
										<div class="row">
											<div class="panel panel-default">
												<table class="table table-fixed">
													<thead>
														<tr>
															<th class="col-xs-1">Faculty ID</th><th class="col-xs-1">Application Number</th><th class="col-xs-1">Leave ID</th><th class="col-xs-2">Start Date</th><th class="col-xs-2">End Date</th><th class="col-xs-2">Suffix</th><th class="col-xs-2">Prefix</th><th class="col-xs-1">Status</th>
														</tr>
													</thead>
													<tbody>
														<?php
															$i=1;
															$sql = 'SELECT fid FROM faculty_details';
															$result2 = mysqli_query($conn, $sql);
															...
															while($row2 = mysqli_fetch_assoc($result2)){
																...
																while($row = mysqli_fetch_assoc($result)){		
																	...
																	echo '<td class="col-xs-1">'.$row["app_no"].'</td>';
																	echo '<td class="col-xs-1">'.$row["leave_id"].'</td>';
																	echo '<td class="col-xs-2">'.$row["start"].'</td>';
																	echo '<td class="col-xs-2">'.$row["end"].'</td>';
																	echo '<td class="col-xs-2">'.$row["suffix_nodays"].'</td>';
																	echo '<td class="col-xs-2">'.$row["prefix_nodays"].'</td>';
								...
								<?php
									if(isset($_POST["assign"])){
										...
										$sql = 'INSERT INTO roles VALUES ("Dealing Clerk", '.$_POST["new_role"].', "'.$_POST["start_role"].'", "'.$_POST["end_role"].'", '.$user.', '.$old_role["role"].')';
										if(mysqli_query($conn, $sql)){
											$sql = 'UPDATE faculty_details SET role = "Dealing Clerk" WHERE fid = '.$_POST["new_role"].'';
