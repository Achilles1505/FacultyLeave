							...
							<div class="tab-content">
									...
									<div class="container" style="width: 95%">
										<div class="panel-group" id="accordion">
											<?php
												...
												$sql = 'SELECT app_no FROM pending_tasks WHERE role = "Dean" and complete = FALSE';
												$result = mysqli_query($conn, $sql);
												$i=0; $j=0;
												while($row = mysqli_fetch_assoc($result)){
													...
													if(isset($_POST['forward'.$i.''])){
														...
														$sql = 'UPDATE work_flow SET role1 = "Dean", role2 = "Dealing Clerk" WHERE app_no = '.$row["app_no"].'';
														if(mysqli_query($conn, $sql))
														...
														$sql = 'INSERT INTO pending_tasks VALUES ("Director", '.$row["app_no"].', NULL, FALSE)';
														if(mysqli_query($conn, $sql))
															echo "Inserted in pending tasks";
														else
															echo "Could not insert into pending tasks";
														
														$sql = 'UPDATE work_flow SET role1 = "Dean", role2 = "Director" WHERE app_no = '.$row["app_no"].'';
														if(mysqli_query($conn, $sql))
															echo "Updated Successfully";
														else
															echo "Could not update.";
													...
													?>
														...
														echo '</table>';
													?>
																		<form action = "welcome_dean.php" method = "post">
																		<?php
																			
																				echo '<button type = "submit" class = "button" name = "approve'.$i.'">Forward to Director</button>';
													
																				echo '<button type = "submit" class = "button" name = "forward'.$i.'">Forward to FA</button>';
								...
								<?php
									if(isset($_POST["assign"])){
										...
										$sql = 'INSERT INTO roles VALUES ("Dean", '.$_POST["new_role"].', "'.$_POST["start_role"].'", "'.$_POST["end_role"].'", '.$user.', '.$old_role["role"].')';
										if(mysqli_query($conn, $sql)){
